...
from course1.planar_utils import plot_decision_boundary, load_planar_dataset, load_extra_datasets
...
def forward_propagation(X, parameters):
    from course1.week2 import sigmoid
    """
    Argument:
...
def compute_cost(A2, Y, parameters):
    ...
    cost = np.multiply((-1/m), np.sum(np.add(np.multiply(Y, np.log(A2)), np.multiply((1-Y), np.log(1-A2)))))# compute cost
    ### END CODE HERE ###
